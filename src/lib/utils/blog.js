...
export const getArticles = async () => {
  ...
  const rawArticlesContent = fs.readdirSync(location)
  .filter((file) => file.match(/.*\.(svx?)/ig)) //get all markdown files
  .map((file) => {
    return {
      fileName: path.parse(file).name,
  ...
  return await Promise.all(rawArticlesContent.map(async (article) => {
    ...
    return { title, slug: article.fileName, preview: shortDescription || seoTitle, image, shortDescription, isPublished};
  }))
}
